...
use crate::{cfg::I2cSlaveProperties, generate_for_each_macro};

/// Instance configuration, used in [device.i2c_slave.instances]
#[derive(Debug, Default, Clone, serde::Deserialize, serde::Serialize)]
pub(crate) struct I2cSlaveInstanceConfig {
...
/// Generates `for_each_i2c_slave!` which can be used to implement the I2C
/// slave Instance trait for the relevant peripherals. The macro generates code
/// for each [device.i2c_slave.instances[X]] instance.
pub(crate) fn generate_i2c_slave_peripherals(i2c: &I2cSlaveProperties) -> TokenStream {
    let i2c_slave_instance_cfgs = i2c
        .instances
        .iter()
        .map(|instance| {
    ...
    let for_each = generate_for_each_macro("i2c_slave", &[("all", &i2c_slave_instance_cfgs)]);
